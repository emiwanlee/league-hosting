...
const MP = ["MP", 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28];
const MW = ["MW", 20, 19, 19, 17, 16, 15, 12, 11, 12, 12, 11, 11, 9, 7, 7, 8, 5, 6, 4, 3];
const MD = ["MD", 4, 7, 6, 5, 5, 2, 8, 9, 5, 4, 6, 5, 8, 8, 5, 7, 7, 7, 5, 5];
const GF = ["GF", 70, 65, 63, 60, 59, 39, 46, 50, 42, 59, 47, 43, 41, 33, 41, 29, 42, 35, 29, 24];
const GA = ["GA", 24, 26, 28, 42, 42, 39, 50, 44, 44, 48, 45, 44, 52, 48, 54, 39, 60, 51, 63, 74];
...
let teams = ["Teams", "Arsenal", "Liverpool", "Manchester City", "Aston Villa", "Tottenham", "Manchester United",
 "West Ham", "Brighton", "Wolves", "Newcastle", "Chelsea", "Fulham", "Bournemouth", 
 "Crystal Palace", "Brentford", "Everton", "Luton", "Nottingham", "Burnley", "Sheffield United"];
...
//This is the code for % win, draw, loss calculation
   const win_ratio_arsenal = (MW[1] / MP[1] ) * 100;
   const draw_ratio_arsenal = (MD[1] / MP[1] ) * 100;
   const loss_ratio_arsenal = (ML[1] / MP[1] ) * 100;


   const win_ratio_liverpool = (MW[2] / MP[2] ) * 100;
   const draw_ratio_liverpool = (MD[2] / MP[2] ) * 100;
   const loss_ratio_liverpool = (ML[2] / MP[2] ) * 100;

   const win_ratio_man_city = (MW[3] / MP[3] ) * 100;
   const draw_ratio_man_city = (MD[3] / MP[3] ) * 100;
   const loss_ratio_man_city = (ML[3] / MP[3] ) * 100;

   const win_ratio_aston_villa = (MW[4] / MP[4] ) * 100;
   const draw_ratio_aston_villa = (MD[4] / MP[4] ) * 100;
   const loss_ratio_aston_villa = (ML[4] / MP[4] ) * 100;

   const win_ratio_tottenham = (MW[5] / MP[5] ) * 100;
   const draw_ratio_tottenham = (MD[5] / MP[5] ) * 100;
   const loss_ratio_tottenham = (ML[5] / MP[5] ) * 100;

   const win_ratio_man_utd = (MW[6] / MP[6] ) * 100;
   const draw_ratio_man_utd = (MD[6] / MP[6] ) * 100;
   const loss_ratio_man_utd = (ML[6] / MP[6] ) * 100;

   const win_ratio_west_ham = (MW[7] / MP[7] ) * 100;
   const draw_ratio_west_ham = (MD[7] / MP[7] ) * 100;
   const loss_ratio_west_ham = (ML[7] / MP[7] ) * 100;

   const win_ratio_brighton = (MW[8] / MP[8] ) * 100;
   const draw_ratio_brighton = (MD[8] / MP[8] ) * 100;
   const loss_ratio_brighton = (ML[8] / MP[8] ) * 100;

   const win_ratio_newcastle = (MW[10] / MP[10] ) * 100;
   const draw_ratio_newcastle = (MD[10] / MP[10] ) * 100;
   const loss_ratio_newcastle = (ML[10] / MP[10] ) * 100;

   const win_ratio_wolves = (MW[9] / MP[9] ) * 100;
   const draw_ratio_wolves = (MD[9] / MP[9] ) * 100;
   const loss_ratio_wolves = (ML[9] / MP[9] ) * 100;
   

   const win_ratio_chelsea = (MW[11] / MP[11] ) * 100;
   const draw_ratio_chelsea = (MD[11] / MP[11] ) * 100;
   const loss_ratio_chelsea = (ML[11] / MP[11] ) * 100;

   const win_ratio_crystal_palace = (MW[14] / MP[15] ) * 100;
   const draw_ratio_crystal_palace = (MD[14] / MP[1] ) * 100;
   const loss_ratio_crystal_palace = (ML[14] / MP[14] ) * 100;

   const win_ratio_fulham = (MW[12] / MP[12] ) * 100;
   const draw_ratio_fulham = (MD[12] / MP[12] ) * 100;
   const loss_ratio_fulham = (ML[12] / MP[12] ) * 100;

   const win_ratio_bournemouth = (MW[13] / MP[13] ) * 100;
   const draw_ratio_bournemouth = (MD[13] / MP[13] ) * 100;
   const loss_ratio_bournemouth = (ML[13] / MP[13] ) * 100;

   const win_ratio_brentford = (MW[15] / MP[15] ) * 100;
   const draw_ratio_brentford = (MD[15] / MP[15] ) * 100;
   const loss_ratio_brentford = (ML[15] / MP[15] ) * 100;

   const win_ratio_nottingham = (MW[18] / MP[18] ) * 100;
   const draw_ratio_nottingham = (MD[18] / MP[18] ) * 100;
   const loss_ratio_nottingham = (ML[18] / MP[18] ) * 100;

   const win_ratio_luton = (MW[17] / MP[17] ) * 100;
   const draw_ratio_luton = (MD[17] / MP[17] ) * 100;
   const loss_ratio_luton = (ML[17] / MP[17] ) * 100;

   const win_ratio_everton = (MW[16] / MP[16] ) * 100;
   const draw_ratio_everton = (MD[16] / MP[16] ) * 100;
   const loss_ratio_everton = (ML[16] / MP[16] ) * 100;

   const win_ratio_burnley = (MW[19] / MP[19] ) * 100;
   const draw_ratio_burnley = (MD[19] / MP[14] ) * 100;
   const loss_ratio_burnley = (ML[19] / MP[19] ) * 100;


   const win_ratio_sheffield = (MW[20] / MP[20] ) * 100;
   const draw_ratio_sheffield = (MD[20] / MP[20] ) * 100;
   const loss_ratio_sheffield = (ML[20] / MP[20] ) * 100;
...
document.getElementById("lut-wr").innerHTML = win_ratio_luton.toFixed(0) + "%";
document.getElementById("lut-dr").innerHTML = draw_ratio_luton.toFixed(0) + "%";
document.getElementById("lut-lr").innerHTML = loss_ratio_luton.toFixed(0) + "%";
...
document.getElementById("bur-wr").innerHTML = win_ratio_burnley.toFixed(0) + "%";
document.getElementById("bur-dr").innerHTML = draw_ratio_burnley.toFixed(0) + "%";
document.getElementById("bur-lr").innerHTML = loss_ratio_burnley.toFixed(0) + "%";
...
document.getElementById("wes-wr").innerHTML = win_ratio_west_ham.toFixed(0) + "%";
document.getElementById("wes-dr").innerHTML = draw_ratio_west_ham.toFixed(0) + "%";
document.getElementById("wes-lr").innerHTML = loss_ratio_west_ham.toFixed(0) + "%";
